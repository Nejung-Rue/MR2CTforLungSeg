...
data_path = config['dataset_config']['root_path']
...
class StudyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        source_norm_slice = sitk.ReadImage(os.path.join(data_path, source_norm_slice_filename))
        source_norm_img = sitk.ReadImage(os.path.join(data_path, source_norm_img_filename))
        source_edge = cv2.imread(os.path.join(data_path, source_edge_filename), 0)
        target = sitk.ReadImage(os.path.join(data_path, target_filename))
        target_norm_le1 = sitk.ReadImage(os.path.join(data_path, target_norm_le1_filename))
        target_norm_le2 = sitk.ReadImage(os.path.join(data_path, target_norm_le2_filename))
